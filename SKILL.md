...
扫描当前会话（含最近相关上下文），按语义将信息分为三级：

| 级别 | 判定标准（会话重启后是否还需要） | 落盘位置 | 示例 |
|---|---|---|---|
| 核心级 | 重启后仍必须知道 | 根目录 MEMORY.md | 决策、项目状态、核心知识点、长期偏好、关键联系人 |
| 日常级 | 近期有效 / 需追踪 | memory/YYYY-MM-DD.md | 任务、待办、当日事件、进展 |
| 临时级 | 一次性信息 | 只留当日，过期提示归档 | 验证码、临时链接、一次性数据 |
...
| `autoReset` | false | 自动 /reset（**永远 false**，本技能不提供自动重置） |
